...
if __name__ == "__main__":
    ...
    env = gym.make("FrankaCubeRelocation-Vision-v0", save_video=False)
    env = AVPIntervention(env, gripper_only=True, avp_ip = "192.168.1.10", debug=False)
    ...
    while (
        ...
        # Always refresh the progress bars to show current status
        fw_pbar.set_description(f"fw ({len(fw_transitions)}/{transitions_needed})")
        bw_pbar.set_description(f"bw ({len(bw_transitions)}/{transitions_needed})")
